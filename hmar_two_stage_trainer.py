...
class HMARTwoStageTrainer(Trainer):
    def __init__(
        self,
        stage: str,
        device,
        ...
        for inp_B3HW, label_B in ld_val:
            ...
            if self.stage == "ns":
                with torch.autocast("cuda", enabled=True, cache_enabled=True):
                    logits_BLV = self.transformer_wo_ddp(label_B, x_ns, None, mode="ns")
                ...
                with torch.autocast("cuda", enabled=True, cache_enabled=True):
                    logits_BLV = self.transformer_wo_ddp(label_B, x, idx_to_mask, mode="mask")
                L_mean += self.val_loss(
                    logits_BLV[:, idx_to_mask_plus_1, :].reshape(-1, V),
                    gt_BL[:, idx_to_mask_plus_1].reshape(-1),
                ) * B
                L_tail += self.val_loss(
                    logits_BLV.data[:, idx_to_mask_list_plus_1[-1], :].reshape(-1, V),
                    gt_BL[:, idx_to_mask_list_plus_1[-1]].reshape(-1),
                ) * B
                acc_mean += (logits_BLV.data[:, idx_to_mask_plus_1].argmax(dim=-1) == gt_BL[:, idx_to_mask_plus_1]).sum() * (
                    100 / idx_to_mask_plus_1.shape[0]
                )
                acc_tail += (logits_BLV.data[:, idx_to_mask_list_plus_1[-1]].argmax(dim=-1) == gt_BL[:, idx_to_mask_list_plus_1[-1]]).sum() * (
                    100 / idx_to_mask_list_plus_1[-1].shape[0]
                )
                ...
                for si, (bg, ed) in enumerate(self.begin_ends):
                    L_resos[si] += self.val_loss(
                        logits_BLV.data[:, idx_to_mask_list_plus_1[si], :].reshape(-1, V),
                        gt_BL[:, idx_to_mask_list_plus_1[si]].reshape(-1),
                    ) * B
                    acc_resos[si] += (logits_BLV.data[:, idx_to_mask_list_plus_1[si]].argmax(dim=-1) == gt_BL[:, idx_to_mask_list_plus_1[si]]).sum() * (
                        100 / (ed - bg)
                    )
            tot += B
    ...
    def train_step(
        ...
        if self.stage == "ns":
            with self.optimizer.amp_ctx:
                with torch.autocast("cuda", enabled=True, cache_enabled=True):
                    logits_BLV = self.transformer(label_B, x_ns, None, mode="ns")
        ...
        with self.optimizer.amp_ctx:
            with torch.autocast("cuda", enabled=True, cache_enabled=True):
                logits_BLV = self.transformer(label_B, x, idx_to_mask, mode="mask")
            loss = self.train_loss(logits_BLV[:, idx_to_mask_plus_1, :].reshape(-1, V), gt_BL[:, idx_to_mask_plus_1].reshape(-1)).view(B, -1)
        ...
        for k in ("transformer_wo_ddp", "vae_local", "optimizer"):
            ...
            if m is not None:
                ...
                if ret is not None:
                    ...
                    print(f"[HMARTwoStageTrainer.load_state_dict] {k} missing:  {missing}")
                    print(f"[HMARTwoStageTrainer.load_state_dict] {k} unexpected:  {unexpected}")
